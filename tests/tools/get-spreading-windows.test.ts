...
describe('get_spreading_windows tool', () => {
  ...
  test('returns windows for lisier (type II)', () => {
    const result = handleGetSpreadingWindows(db, { manure_type: 'lisier', land_type: 'grandes cultures' });
  ...
  test('returns windows for fumier (type I)', () => {
    const result = handleGetSpreadingWindows(db, { manure_type: 'fumier', land_type: 'grandes cultures' });
    expect(result).toHaveProperty('windows');
    const typed = result as { windows: Array<{ closed_period: { start: string; end: string } | null }> };
    expect(typed.windows.length).toBeGreaterThan(0);
  });
  ...
  test('returns not_found for unknown manure type', () => {
    const result = handleGetSpreadingWindows(db, { manure_type: 'uranium', land_type: 'prairies' });
    expect(result).toHaveProperty('error', 'not_found');
  });

  test('rejects unsupported jurisdiction', () => {
    const result = handleGetSpreadingWindows(db, { manure_type: 'lisier', land_type: 'prairies', jurisdiction: 'NL' });
